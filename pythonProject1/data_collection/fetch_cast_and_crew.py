...
# -------------------------
# Fetch all movie IDs from movies_discovered
# -------------------------
cur.execute("SELECT id FROM movies")
movie_ids = [row[0] for row in cur.fetchall()]

print(f"Found {len(movie_ids)} movies in movies_discovered to fetch cast & crew")
...
for movie_id in movie_ids:
    if movie_id>627:
        ...
        try:
            ...
            conn.commit()
            time.sleep(0.5)  # avoid hammering TMDb
